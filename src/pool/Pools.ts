...
//     if (!pools?.length) {
//       throw new Error('[Pools][release] pools should not be empty')
//     }
//     this._pools = pools
...
//     const pools = this._pools
//     for (let i = 0, len = pools.length; i < len; i++) {
//       pools[i].hold(typeof count === 'number' ? count : count[i])
//     }
//     return true
//   }
//
...
//       }
//       else {
//         throw new Error(`[Pools][release] count (${count} > maxReleaseCount (${maxReleaseCount}))`)
//       }
//     }
//     if (count > 0) {
//       const pools = this._pools
...
//         throw new Error(`[Pools][holdWait] count (${count}) > maxSize (${this.maxSize}))`)
//       }
//     } else {
//       for (let i = 0, len = count.length; i < len; i++) {
//         if (count[i] > this._pools[i].maxSize) {
//           throw new Error(`[Pools][holdWait] count[${i}] (${count[i]} > pools[i].maxSize (${this._pools[i].maxSize})`)
//         }
//       }
//     }
//
